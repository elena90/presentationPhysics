...
$(window).on('ChangeSlide', function (evt, number) {
    ...
    var $innerContainer = $currentSlide.find('.js-example4');

    if($innerContainer.length) {
        startExample4();
        
    } else {
        $.Window.trigger('stopExample4');
    }
});


function startExample4() {
    var $obj = $('.Story-wrapper--2');
    var $allEls = $('.Story-wrapper--2, .Story-navPrev, .Story-navNext')
    ...
    $.Window.on('stopExample4', function () {
        stopAnimate();
        ...
        $navPrev.off('mousedown');
        $navNext.off('mousedown');
    });
