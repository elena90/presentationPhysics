...
$(window).on('ChangeSlide', function (evt, number) {
    ...
    var $innerContainer = $currentSlide.find('.js-example20');

    if($innerContainer.length) {
        startExample20();
        
    } else {
        $.Window.trigger('stopExample20');
    }
});


function startExample20() {
    var $obj = $('.ball20');
    var $floor = $('.floor40');
    ...
    $.Window.on('stopExample20', function () {
        stopAnimate();
    });
    ...
    function init() {
        ...
        obj.pos = new Vector(512, 50);
        obj.changeStyles();
        obj.velo = new Vector(0, 0)

        // Рисуем пол
        var b1 = new Vector(0, 600);
        var b2 = new Vector(1024, 600);
