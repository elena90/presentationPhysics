...
$(window).on('ChangeSlide', function (evt, number) {
    ...
    var $innerContainer = $currentSlide.find('.js-example9');

    if($innerContainer.length) {
        startExample9();
        
    } else {
        $.Window.trigger('stopExample9');
    }
});


function startExample9() {
    ...
    var collectImage = {
        init: function ($el, ops) {
            ...
            $.Window.on('stopExample9', function () {
                _this.stopAnimate();
                _this.$el.empty();
            });
        },
    ...
    for (var i = 0; i < countX; i++) {
        for (var j = 0; j < countY; j++) {
            var $item = $('<div class="MovingImage-item"></div>');
            $item.css({
                width: sizeEl,
                height: sizeEl,
                top: j * sizeEl,
